...
func debugEngraveFlow(ctx *Context, ops op.Ctx) {
	...
	for {
		...
		p.Wakeup()
		drawDebug(ctx, ops, lastSt, eerr)
		ctx.Frame()
	}
}

func drawDebug(ctx *Context, ops op.Ctx, st EngraverStatus, eerr string) {
	dims := ctx.Platform.DisplaySize()
	th := &descriptorTheme
	op.ColorOp(ops, th.Background)
	...
	widget.Labelwf(ops.Begin(), ctx.Styles.body, r.Dx(), th.Text,
		"X Speed: %dmm/s\nY Speed: %dmm/s\nX Load: %d\nY Load: %d\nError: %s",
		st.XSpeed, st.YSpeed, st.XLoad, st.YLoad, eerr)
