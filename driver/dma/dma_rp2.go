...
func ChannelAt(ch uint8) *Channel {
	return &channels[ch]
}
...
func SetInterrupt(ch uint8, callback func()) error {
	mu.Lock()
	defer mu.Unlock()
	if callback != nil {
...
func ForceInterrupt(ch uint8) {
	num := irqForChannel(ch)
	irqs[num].INTF.SetBits(0b1 << ch)
}

func ClearForceInterrupt(ch uint8) {
	num := irqForChannel(ch)
	irqs[num].INTF.ClearBits(0b1 << ch)
}

func irqForChannel(ch uint8) uint8 {
